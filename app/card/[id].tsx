...
import { isFoilLockedCard } from '../../src/features/scan/scan-match.service';
...
function getFallbackCard(params: Partial<Record<keyof CardRouteParams, string | string[]>>) {
...
export default function CardDetailScreen() {
  ...
  async function openCardmarket(card: RiftboundCard) {
    ...
    const shouldUseDirectCardmarket = !card.matchConfidence || card.matchConfidence === 'exact';
  ...
  async function handleQuickAdd(card: RiftboundCard, printing: CollectionPrinting) {
  ...
  async function handleQuickRemove(card: RiftboundCard, printing: CollectionPrinting) {
  ...
  function renderCard({ item }: { item: RiftboundCard }) {
    ...
    const pricePrinting = isFoilLockedCard(item) || (ownership.foil > 0 && ownership.normal === 0)
      ? 'foil'
      : 'normal';
    return (
      <ScrollView
      ...
      >
        <View style={styles.imageWrap}>
          {item.imageUrl ? (
            <Image
              ...
              style={[styles.image, isBattlefieldCard(item) && styles.battlefieldImage]}
            />
          ) : null}
        </View>
        <View style={styles.heroRow}>
          <Text numberOfLines={1} style={styles.name}>{item.name}</Text>
          <Text style={styles.meta}>{formatCardMeta(item.setCode, item.number)}</Text>
          <Text
            numberOfLines={1}
            style={[
        ...
        <View style={styles.priceGrid}>
          <PriceChip label="Low" value={getDisplayPrice(price, 'low', pricePrinting)} />
          <PriceChip label="Avg" value={getDisplayPrice(price, 'avg', pricePrinting)} />
          <PriceChip label="Trend" value={getDisplayPrice(price, 'trend', pricePrinting)} />
        </View>
    ...
    <View style={styles.container}>
      ...
      {activeCard ? (
        <View style={styles.bottomBar}>
          <Button
            label={activeDirectCardmarketUrl ? 'OPEN CARDMARKET' : 'SEARCH CARDMARKET'}
