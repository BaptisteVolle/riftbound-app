...
import {
  ...
  parseCardText as parseCardTextFromParser,
} from "./ocr-parser.service";
import { cropScanImage } from "./scan-debug.service";
...
const TYPE_LABEL_PATTERN =
  /^(champion|unit|spell|rune|gear|battlefield|legend)\b/i;
...
export function parseCardText(text: string): CardScanInput {
  return parseCardTextFromParser(text);
}
